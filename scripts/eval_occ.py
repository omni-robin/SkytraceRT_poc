#!/usr/bin/env python3
"""Evaluate occupancy model vs SigMF GT bands over a directory.

This runs capture-level inference (average over windows) and compares predicted bands
against GT bands in the SigMF meta (absolute Hz).

Metrics (per capture):
- gt_n, pred_n
- best_match_mean_iou: mean over GT of best IoU to any pred
- gt_coverage: fraction of GT bandwidth covered by predicted bands

Designed to be Apple Silicon friendly (uses MPS if available).
"""
...
def infer_capture_occ(
    ...
    for start in range(0, L - win_len + 1, hop):
        ...
        xt = torch.from_numpy(w[None, ...]).to(device)
...
def main() -> int:
    ...
    ap.add_argument("--in-dir", required=True, help="Dir containing *.sigmf-meta/*.sigmf-data")
    ...
    ap.add_argument("--freq-bins", type=int, default=1024)
    ...
    for meta_path in sorted(in_dir.glob("*.sigmf-meta")):
        ...
        occ = infer_capture_occ(model, device, x, args.win_len, args.win_hop)
        ...
        for r in rc.get("rcs") or []:
            gt_bands.append(Band2(lower_hz=float(r["min_frequency_mhz"]) * 1e6, upper_hz=float(r["max_frequency_mhz"]) * 1e6))

        pred_simple = [Band2(lower_hz=b.lower_hz, upper_hz=b.upper_hz) for b in pred_bands]

        # for each gt, best IoU with any pred
        best_ious = []
        for g in gt_bands:
            if not pred_simple:
                best_ious.append(0.0)
            else:
                best_ious.append(max(band_iou_1d(g, p) for p in pred_simple))
